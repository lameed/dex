...
class PartController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Part']))
		{
			...
			$model->manufacturerID = $_POST['Manufacturer']['id'];
			$model->supplierID = $_POST['Supplier']['id'];
			$model->equipmentID = $_POST['Equipment']['id'];
			$model->DateCreated=new CDbExpression('NOW()');
			$model->LastUpdated=new CDbExpression('NULL');
			$model->image=CUploadedFile::getInstance($model, 'image');
			$ext=substr($model->image,-4);
			if($model->save())
				{
				$model->image->saveAs('images/' . $model->id . $ext);
